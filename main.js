...
setInterval(() => {
    const days = Math.floor((timer.targetDate - new Date()) / (1000 * 60 * 60 * 24));
 ...
 */
    function hours() {
        const mathHour = Math.floor(((timer.targetDate - new Date()) % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
        if (mathHour < 10) {
        return '0' + mathHour
        } else {
            return mathHour
 ...
 */
    ...
    function mins() {
        const mathMins = Math.floor(((timer.targetDate - new Date()) % (1000 * 60 * 60)) / (1000 * 60));
        if (mathMins < 10) {
        return '0' + mathMins
        } else {
            return mathMins
 ...
 */
    ...
    function secs() {
        const mathSecs = Math.floor(((timer.targetDate - new Date()) % (1000 * 60)) / 1000);
        if (mathSecs < 10) {
        return '0' + mathSecs
        } else {
            return mathSecs
